...
static void addEquilateralPrism(
    ...
    auto addQuad = [&](const glm::vec3& p0, const glm::vec3& p1, const glm::vec3& p2, const glm::vec3& p3,
        ...
        vertices.push_back({w0, glm::vec2(uv.x, uv.w), n, vertColor});
        vertices.push_back({w1, glm::vec2(uv.z, uv.w), n, vertColor});
        vertices.push_back({w2, glm::vec2(uv.z, uv.y), n, vertColor});
        vertices.push_back({w3, glm::vec2(uv.x, uv.y), n, vertColor});
    ...
    auto addTriangle = [&](const glm::vec3& p0, const glm::vec3& p1, const glm::vec3& p2,
        ...
        vertices.push_back({w0, glm::vec2(uv.x, uv.w), n, vertColor});
        vertices.push_back({w1, glm::vec2(uv.z, uv.w), n, vertColor});
        vertices.push_back({w2, glm::vec2(uv.x + (uv.z - uv.x) * 0.5f, uv.y), n, vertColor});
...
static void addItemQuad(
    ...
    vertices.push_back({w1, glm::vec2(uv.z, uv.w), -n, vertColor * 0.9f});
    vertices.push_back({w0, glm::vec2(uv.x, uv.w), -n, vertColor * 0.9f});
    vertices.push_back({w3, glm::vec2(uv.x, uv.y), -n, vertColor * 0.9f});
    vertices.push_back({w2, glm::vec2(uv.z, uv.y), -n, vertColor * 0.9f});
...
void PlayerModelRenderer::buildModelMesh(const Player& player, float skylight, float torchlight) {
    std::vector<ChunkVertex> vertices;
    std::vector<uint32_t> indices;
    ...
    glm::vec3 vertColor(skylight, 0.85f, torchlight);
    ...
    glm::mat4 rArmMat = playerBase * glm::translate(glm::mat4(1.0f), rShoulderPivot);
    ...
    if (heldBlock != BlockType::Air) {
        ...
        if (Cell{heldBlock}.isItem()) {
            ...
            addItemQuad(vertices, indices, toolMat, 0.32f, blockTile, vertColor);
        } else {
            glm::mat4 blockMat = rArmMat *
                                 glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.02f, 0.20f)) *
                                 glm::rotate(glm::mat4(1.0f), glm::radians(25.0f), glm::vec3(1.0f, 1.0f, 0.0f));
            addEquilateralPrism(vertices, indices, blockMat, 0.22f, 0.22f, blockTile, blockTile, blockTile, blockTile, vertColor);
        }
    }

    glm::vec3 lHipPivot = crouching ? glm::vec3(0.14f, 0.70f, -0.12f) : glm::vec3(0.14f, 0.70f, 0.0f);
    glm::mat4 lLegMat = playerBase *
                        glm::translate(glm::mat4(1.0f), lHipPivot) *
                        glm::rotate(glm::mat4(1.0f), walkSwing + (crouching ? glm::radians(-20.0f) : 0.0f), glm::vec3(1.0f, 0.0f, 0.0f)) *
                        glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -0.70f, 0.0f));
    ...
    glm::vec3 rHipPivot = crouching ? glm::vec3(-0.14f, 0.70f, -0.12f) : glm::vec3(-0.14f, 0.70f, 0.0f);
    ...
    m_isMeshBuilt[f] = true;
}

void PlayerModelRenderer::renderShadow(VkCommandBuffer cmd,
...
void PlayerModelRenderer::resetFrame() {
    m_isMeshBuilt[m_cmdQueue.getCurrentFrame()] = false;
}
...
void PlayerModelRenderer::render(VkCommandBuffer cmd,
    ...
    uint32_t f = m_cmdQueue.getCurrentFrame();
    if (!m_isMeshBuilt[f]) {
        buildModelMesh(player, skylight, torchlight);
    }
